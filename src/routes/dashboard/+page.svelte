...
<div class="flex flex-col content-center md:w-1/5">
	<h1 class="text-4xl size-fit m-2">Dashboard</h1>
	<h2 class="m-2">Welcome, {firstName}</h2>
</div>

<div class="flex flex-col flex-grow">
<div class="flex flex-row bg-sky-900 items-center rounded-lg m-2 overflow-x-auto">
	<svg class="block rounded-lg p-px m-2 flex-grow min-w-[550px]" height="100" role="img">
		<g transform="translate(0, 15)" class="block">
		...
		<g transform="translate(0, 55)" class="block">
	...
	<div class="w-full w-1/2 flex flex-row items-center">
	<h2>Audits ratio:</h2>
...
<svg class="block bg-sky-900 m-2 rounded-lg pt-4 grow min-w-[900px] min-h-[700px] overflow-y-auto" height="540">
	{#each data.XP_Transfers as project, i}
		<g transform={`translate(0, ${i * 30})`} class="group">
			<rect
				width={project.amount / 200}
				height="20"
				class="fill-current text-amber-500 hover:text-amber-300 transition-all block"
			></rect>
